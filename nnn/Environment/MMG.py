...
def activate(ip,delta_d,delta_rps,W_flag,WHA,RuPos,PropPos):
    """
    ----------
    ip      : current state of the ship [u,v,r,x,y,psi]
    RuPos   : Rudder Position
    rps_c   : propeller current speed
    delta_r : commanded rudder angle
    rps_r   : commanded propeller speed
    WHA     : Wave Head Angle
    W_Flag  : Wave Force Inclusion
    Returns
    -------
    Time derivative of ship state[u_dot,v_dot,r_dot,U,V,r], Rudder_Position, Propeller_speed
    ...
    if abs(delta) > np.deg2rad(35):
        delta    = np.sign(delta)*np.deg2rad(35) # Assertion of maximum rudder angle
        
    rps        = PropPos + delta_rps
    if rps < 5:
        rps = 5.0
    if rps > 25:
        rps = 25
    ...
    return nxt_state.tolist(), RuPos_new, rps_new
...
# delta = np.deg2rad(0)
# for i in range(N):
#     temp,RuPos_New,_ = activate(data[-1],-delta,0.0,W_F,WHA,0.0,RPS) # ip,delta_d,W_flag,WHA,RuPos,rps
#     RuPos           = RuPos_New
#     data.append(temp)
...
# for j in range(N):
#     #delta = np.random.rand()*np.deg2rad(20)
#     temp1,RuPos_New,_  = activate(data1[-1],delta,0.0,W_F,WHA,0.0,RPS) 
#     RuPos1           = RuPos_New
#     data1.append(temp1)
...
# plt.title("L3 : KVLCC2 simulation for 120 seconds for n= 18.2 rps with u_0 = 0.78 m/s ")
# plt.legend(loc="best")
# plt.grid()
# plt.show()
# plt.plot(RRR)
